...
{
   eval "require $opts{renderer}";
   die $@ if ($@);
   no strict 'refs';
   ${$opts{renderer}."::xdensity"} = $opts{xdensity} if (defined $opts{xdensity});
   ${$opts{renderer}."::ydensity"} = $opts{ydensity} if (defined $opts{ydensity});
}
